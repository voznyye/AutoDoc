...
**Автоматические коммиты отключены, но версии изменяются автоматически!**

Теперь скрипты:
- ✅ **Автоматически изменяют версию** в package.json
- ✅ **Компилируют и упаковывают** плагин
- ❌ **НЕ делают git коммиты** автоматически
- ❌ **НЕ создают git теги** автоматически

Вы должны вручную:
- Делать git коммиты
- Создавать git теги

## 🚀 Скрипты релиза (без автоматизации)

### Основные команды релиза

#### `npm run release:patch`
- **Автоматически увеличивает версию** на patch (например: 2.3.1 → 2.3.2)
- Компилирует код
- Создает .vsix пакет

#### `npm run release:minor`
- **Автоматически увеличивает версию** на minor (например: 2.3.1 → 2.4.0)
- Компилирует код
- Создает .vsix пакет

#### `npm run release:major`
- **Автоматически увеличивает версию** на major (например: 2.3.1 → 3.0.0)
- Компилирует код
- Создает .vsix пакет
...
1. **Изменение версии**: `npm version` автоматически обновляет package.json
2. **Компиляция**: `npm run compile` собирает TypeScript код
3. **Упаковка**: `npm run package` создает .vsix файл
4. **Результат**: готовый к публикации пакет

**Примечание**: Версия изменяется автоматически, но git операции НЕ выполняются автоматически
...
3. **Релиз**: `npm run release:minor` (или patch/major) - **автоматически изменяет версию**
4. **Коммит**: `git add . && git commit -m "Release v2.4.0"`
5. **Ручная публикация**: загрузить .vsix файл в VS Code Marketplace

### Для автоматической публикации:
1. **Разработка**: работа над новыми функциями
2. **Тестирование**: `npm run compile` и `npm run test`
3. **Релиз + Публикация**: `npm run publish:minor` (или patch/major) - **автоматически изменяет версию**
4. **Коммит**: `git add . && git commit -m "Release v2.4.0"`
